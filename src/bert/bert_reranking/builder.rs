...
impl SequenceClassificationModel {
        ...
        V: AsRef<[Vec<&'a str>]>
    {
        queries
            .as_ref()
            .iter()
            .zip(results_set.iter())
            .map(|qa_set| {
                let (query,results) = qa_set;
                (   
                    results,
                    results.iter()
                        .flat_map(|result| {
            ...
            .map(|results_w_text_pair_list| {
                let (results,text_pair_list) = results_w_text_pair_list;
    ...
    /// # }
    /// ```
    pub fn predict<'a, S, V>(&self, qeries: S, results_set: V, logit_index_threshold: u64) -> Vec<Vec<RankedResults>>
    where
        S: AsRef<[&'a str]>,
        V: AsRef<[Vec<&'a str>]>
    {
        let model_inputs = self.prepare_for_model(query.as_ref(),results_set.as_ref())
        model_inputs
            .as_ref()
            .map (|model_input| {
                let (result,input_ids,token_type_ids) = model_input;
                let prob_scores = no_grad(|| {
                    let logits = self.sequence_classifier.forward_t(
                        Some(&input_ids),
                        None,
                        Some(&token_type_ids),
                        None,
                    ...
                    let probs = logits.softmax(-1, Kind::Float).narrow(1,0,logit_index_threshold);
                    probs.f_sum_dim_intlist(&[1], true, Kind::Float).detach().to(Device::Cpu)
                });
                let rankings_index_map = prob_scores.as_ref().argsort(-1,true);
                rankings_index_map
                    .iter()
                    .enumerate()
                    .map(|(rank,index)| {
                        RankedResults {
                            text:results[index],
                            score:prob_scores[index],
                            rank:rank
                        }
                    })
            })
        .collect(Vec<Vec<RankedResults>>);
    }