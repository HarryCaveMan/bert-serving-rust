...
impl RerankingModel {
        ...
        V: AsRef<[Vec<&'a str>]>
    {
        let text_pair_lists: Vec<Vec<(&'a str,&'a str)>> = queries
            .as_ref()
            .par_iter()
            .zip(results_set.as_ref().par_iter())
            .map(|(query,results)| {
                results
                    .par_iter()
                    .map(|result| {
        ...
        text_pair_lists
            ...
            .map(|text_pair_list| {
                ...
                .collect::<Vec<_>>();
                (
                    text_pair_list.par_iter().map(|pair| pair.1).collect::<Vec<&'a str>>(),
        ...
        V: AsRef<[Vec<&'a str>]>
    {
        ...
        RankedResults {
            queries: queries.as_ref().par_iter().map(|q| String::from(*q)).collect::<Vec<String>>(),
